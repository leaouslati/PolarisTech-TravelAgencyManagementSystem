...
export default function MyBookings() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
              ...
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-700 bg-blue-50 dark:bg-blue-900/40">
                  {['Booking ID', 'Package', 'Destination', 'Travel Date', 'Travelers', 'Status', 'Total Price', 'Actions'].map(h => (
                    <th
                      key={h}
                      className="px-6 py-3 text-xs font-semibold text-blue-700 dark:text-blue-300 uppercase tracking-wide text-left align-middle"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {bookings.map((booking) => (
                  <tr key={booking.booking_id} className="transition-colors align-middle">
                    ...
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-400 align-middle">{booking.destination_city}</td>
      ...
      {/* Edit Booking Modal — outside the table */}
      {showEditModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div
            className="absolute inset-0 bg-black/50 backdrop-blur-sm"
            onClick={() => { setShowEditModal(false); setEditRow(null); }}
          />
          <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xl">
            <div className="p-6">
              ...
              <form onSubmit={e => { e.preventDefault(); handleModify(editRow); }}>
                ...
                <div className="mt-6 flex gap-3 justify-end">
                  <button
                    ...
                    className="px-5 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition-colors duration-200 disabled:opacity-60"
                  >
      ...
      {/* Cancel Booking Modal — outside the table */}
      {cancelRow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
