    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-md p-6">
        ...
        <div className="flex gap-3 justify-end">
          <button
            onClick={onCancel}
            className="px-4 py-2 text-sm font-semibold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition"
          >
            Cancel
          </button>
          <button
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-md p-6">
        ...
        <div className="flex gap-3 mt-6 justify-end">
          ...
          <button
            onClick={handleSave}
            disabled={loading}
            className="px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:bg-blue-400 transition"
          >
...
const TH =
  'text-left py-3 px-4 text-xs font-semibold text-slate-800 dark:text-white uppercase tracking-wide';
...
export default function ManageUsers() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeleteError(
        err.response?.data?.message || 'Could not deactivate user. Please try again.'
      );
    } finally {
      ...
      <div className="w-full px-4 sm:px-6 lg:px-10 xl:px-16 py-8">
        <div className="mb-6">
          ...
          <p className="text-base text-slate-500 dark:text-slate-400 mt-2">
            View and manage all platform accounts
          </p>
        </div>
              ...
              <table className="w-full min-w-[900px] text-sm">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700 bg-blue-50 dark:bg-blue-900/40">
                    ...
                    <th className="text-right py-3 px-4 text-xs font-semibold text-blue-700 dark:text-blue-300 uppercase tracking-wide">
                      Actions
                    </th>
                  </tr>
                </thead>
                  ...
                  {filtered.map((u) => (
                    <tr key={u.user_id} className="transition-colors hover:bg-blue-50/40 dark:hover:bg-blue-900/10">
                      ...
                      <td className="py-3 px-4">
                        <span
                          className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                            statusColors[u.status] || 'bg-slate-100 text-slate-700'
                          }`}
                        >
                          {u.status}
                        </span>
